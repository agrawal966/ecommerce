...
class Crud extends CI_Model {
	...
	public function current_plan($user_id) {
		...
		$plan = $this->crud->readData('purchase_history.*, plan_type', 'purchase_history', array('user_id' => $user_id, 'purchase_history.status' => 'Completed'), array('plan' => 'purchase_history.plan_id = plan.plan_id'), NULL, 'p_h_id DESC')->row_array();
		if($plan['plan_type'] == '1') {
			$date = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . ' -'.PAYPAL_PAY_PERIOD.' day'));
			$current_plan = $this->crud->readData('purchase_history.*, plan_type', 'purchase_history', array('user_id' => $user_id, 'purchase_history.status' => 'Completed'), array('plan' => 'purchase_history.plan_id = plan.plan_id'), NULL, 'p_h_id DESC')->row();
			return $current_plan;
	   	}
	   	else {
           	$date = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . ' -'.PAYPAL_PAY_PERIOD.' day'));
